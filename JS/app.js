...
const API_KEY = "ed8df546221c41a0bd5114802242110";
...
const displayHourlyForecast = (hourlyData) => {
    ...
    const next24Hours = new Date(currentHour.getTime() + 24 * 60 * 60 * 1000);
...
// Filter the hourly data to only include the next 24 hours
    ...
    hourlyWeatherDiv.innerHTML = next24HoursData.map(item => {
        const temperature = Math.floor(item.temp_c); //Math.floor removes the decimal from the data
        const time = item.time.split(" ")[1].substring(0, 5); 
        //split the string into date & time. Substring take the first 5 letters from the time so it become: HH:MM.
        const weatherIcon = Object.keys(weatherCodes).find(icon => weatherCodes[icon].includes(item.condition.code)); 

        return `
            <li class="weather-item">
            <p class="time">${time}</p>
            <img src="/img/icons/${weatherIcon}.svg"  class="weather-icon">
...
const getWeatherDetails = async (API_URL) => {
  ...
  try {
    ...
    const response = await fetch(API_URL); // await väntar på fetch
    const data = await response.json(); // await väntar på att JSON ska parsa svaret

    // Extract current weather details
    const temperature = Math.floor(data.current.temp_c); //Math.floor removes the decimal from the data
    const description = data.current.condition.text;
    const weatherIcon = Object.keys(weatherCodes).find(icon => weatherCodes[icon].includes(data.current.condition.code));
    // It finds the key in weatherCodes that matches the item.condition.code

    // console.log("weatherCodes:", data.current.condition.code);

    // Update the current weather display
    currentWeatherDiv.querySelector(
      ".weather-icon"
    ).src = `/img/icons/${weatherIcon}.svg`;
    currentWeatherDiv.querySelector(
      ".temperature"
...
// Initial weather request for Malmö as the default city
setupWeatherRequest("Malmö");
